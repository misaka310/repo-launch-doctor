...
`pyproject.toml`の`version`を上げてCHANGELOG.mdの`## Unreleased`を新しいバージョン見出しへ確定するPRをマージしたら、そのマージコミットへ`vX.Y.Z`タグを打ってpushします。他リポジトリがこのツールを`ref: <SHA>`で固定参照する場合、そのSHAに対応するタグを確認すればCHANGELOG上のどの変更を含むかを追跡できます。

`.github/workflows/repo-launch-doctor.yml`が自己参照する`ref`（PRによる自己改変でgateが弱体化することを防ぐ意図的な固定）も、mainへの安全な変更のマージ後に定期的に最新へ更新してください。
